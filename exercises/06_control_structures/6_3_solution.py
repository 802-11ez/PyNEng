#!/usr/bin/env python3
print('''В скрипте сделан генератор конфигурации для access-портов.
...
trunk_template = [
    ...
    'switchport trunk allowed vlan', 'exit'
    ]
access = {
    '0/12': '10',
    '0/14': '11',
...
for intf, comm_l in trunk.items():
    print('interface FastEthernet' + intf)
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            string, *rest = comm_l
            print(' {} {} {}'.format(command, string, ','.join(rest)))
        elif command != 'exit':
            print(' {}'.format(command))
        else:
            print('{}'.format(command))
